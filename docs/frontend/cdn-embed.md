...
:::info Getting a Key
Log in to [Ozwell Manager](https://ozwellconsole.os.mieweb.org) with MIE auth (`manager.os.mieweb.org` credentials). Ozwell automatically provisions your `ozw_` parent key, then lets you create agents with `agnt_key-` agent keys.
:::
...
Agent keys connect to a server-side agent definition that manages the system prompt, model, temperature, and allowed tools. This is the recommended embed path.
...
3. Create an agent in the manager UI.
4. Copy the **Agent Key** from the response (starts with `agnt_key-`).
5. Use it in the embed config.

### Option B: Parent API Key

Parent keys give you raw chat-completions access. Use this when the page provides the model, system prompt, and tools inline in `window.OzwellChatConfig`.

1. Log in to [Ozwell Manager](https://ozwellconsole.os.mieweb.org) with MIE auth (`manager.os.mieweb.org` credentials).
2. Use **Show key** to copy your parent key (starts with `ozw_`).
3. Put the parent key, model, system prompt, and any page tools in `window.OzwellChatConfig`.

---

## Configuration Options

Configure the loader before loading the script:
...
|-----------------|------|-------------|
| `apiKey` | string | Agent key (`agnt_key-...`) or parent key (`ozw_...`) |
| `endpoint` | string | Chat-completions endpoint; defaults to the loader's origin |
| `title` | string | Title in the host-page widget shell |
| `placeholder` | string | Composer placeholder |
| `welcomeMessage` | string | Initial assistant message |
| `thinkingEnabled` | boolean | Shows the thinking-mode control when reasoning is available |
| `thinkingDefaultMode` | `0` to `3` | `0` hidden, `1` collapsed, `2` auto, `3` expanded |
| `defaultUI` | boolean | Set `false` when mounting into your own container |
| `containerId` | string | Element ID for a custom mount target |
...
### Define Page Tools Inline

Use the agent key generated by Ozwell Manager, then add page-side tools directly in your page config when the agent needs to read from or write to the host page:
...
### Advanced: Parent Key Direct Chat

Parent keys (`ozw_...`) can be used directly for chat completions when the page provides the chat configuration inline. In that mode, put the parent key, model, system prompt, and any page tools in `window.OzwellChatConfig`:

```html
<script>
  window.OzwellChatConfig = {
    apiKey: 'ozw_your-parent-key',
...
For normal embeds, prefer an agent key from Ozwell Manager so the prompt, model policy, and shared tools stay server-side.
...
⚠️ **Every widget instance requires a valid API key.**

Recommended: use Ozwell Manager to create an agent and copy its `agnt_key-...` key. The agent key points to the server-side persona, tools, and model policy for that agent.

Advanced: parent keys (`ozw_...`) can authenticate direct chat-completions/API requests when the page supplies the needed model/system/tool config in `window.OzwellChatConfig`.
...
3. **Verify the agent key** exists in Ozwell Manager, or that your parent-key config includes a model and system prompt
